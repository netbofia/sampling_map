...
{"code":"ES" , "name":"Spain", "value":1, "color":"#d8854f"},
...
{"code":"US" , "name":"United States", "value":4, "color":"#E4C913"},
...
console.log("here "+mapData[61].value);
var map;
var minBulletSize = 5;
...
AmCharts.ready(function() {
	...
	map = new AmCharts.AmMap();
  	map.pathToImages = "assets/img/";

	map.addTitle("My samples", 14);
	map.addTitle("source: [from]", 11);
	map.areasSettings = {
		unlistedAreasColor: "#000000",
		unlistedAreasAlpha: 0.1
	};
	map.imagesSettings.balloonText = "<span style='font-size:14px;'><b>[[title]]</b>: [[value]]</span>";
	console.log("In amcharts");
	var dataProvider = {
...
$xhr=$.ajax({
        dataType: "json",
        url: "http://ebola.dat.thedash.com/api/rows?limit=-1",
})
.done(function(){
        console.log("done getting json");
        $lastRow=$xhr.responseJSON.rows.length-1;

        for (var i in mapData){
                if (mapData[i].name in $xhr.responseJSON.rows[$lastRow]){
                        console.log($xhr.responseJSON.rows[$lastRow][mapData[i].name]);
                        mapData[i].value=$xhr.responseJSON.rows[$lastRow][mapData[i].name].cases
                        console.log(mapData[i].value+" dd");
